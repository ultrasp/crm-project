...
import { CrmPasportPanel2 } from '../CrmPasportPanel2/CrmPasportPanel2';
...
export const CITIZENSHIP_UZBEKISTAN = '182';

@Component({
  selector: 'car-owner-individuals-2',
  templateUrl: './CrmAddCarOwnerIndividuals2.html',
  styleUrls: ['./CrmAddCarOwnerIndividuals2.css']
})
export class CrmAddCarOwnerIndividuals2 implements OnInit, OnDestroy,AfterViewInit {
  ...
  @ViewChild('pasportPanel') pasportPanel!: CrmPasportPanel2;
  ...
  ngOnInit() {
    this.form.get('citizenship_id')?.valueChanges.subscribe(citizenshipId => {
      if(citizenshipId && citizenshipId != CITIZENSHIP_UZBEKISTAN){
        this.form.patchValue({
          seriya:this.pasportDocNo.substring(0,2),
          nomer:this.pasportDocNo.substring(2)
        })
      }
      this.pnflValidationCheck();
    })
  }
  ...
  pnflValidationCheck(){
    // console.log(this.parentForm.get('fizikInfo.citizenship_id')?.value,CITIZENSHIP_UZBEKISTAN,"CITIZENSHIP_UZBEKISTAN")
    if(this.parentForm.get('fizikInfo.citizenship_id')?.value == CITIZENSHIP_UZBEKISTAN){
      this.parentForm.get('fizikInfo.pnfl')?.addValidators(Validators.required);               
    } else {                
      this.parentForm.get('fizikInfo.pnfl')?.clearValidators();               
    }
    this.parentForm.get('fizikInfo.pnfl')?.updateValueAndValidity();       

  }

  pasportChanged(pasport:string){ 
    if(pasport && this.form.get('citizenship_id')?.value && this.form.get('citizenship_id')?.value != CITIZENSHIP_UZBEKISTAN){
      this.form.patchValue({
        seriya:pasport.substring(0,2),
        nomer:pasport.substring(2)
      })
    }
    console.log(pasport, this.form.get('citizenship_id')?.value,'pasport')
  }
  ...
  form: FormGroup = new FormGroup({
    ...
    middlename: new FormControl('', [Validators.required, Validators.maxLength(100)]),
    ...
    givenDate: new FormControl(null, []),
    ...
    born_place:new FormControl(null, [Validators.maxLength(50)]),
    // pnfl: new FormControl(null, [Validators.required, Validators.maxLength(20)]),
    pnfl: new FormControl(null,[]),
    citizenship_id: new FormControl(null,[Validators.required]),
  ...
  onSearch() {
    if(this.pasportDocNo == null || this.pasportDocNo == ''){
      CrmAlertDialog.show('CAR.PLEASE_FILL_PASPORT_NUMBER',CrmAlertDialogTypeError.ERROR)
      return;
    }
  ...
  fillDriverToForm(driver: ICarPersonGet) {
    ...
    this.form.patchValue({
      ...
      givenBy: driver.doc_giveplace_id,
      born_country_id: driver.birth_country_id,
      citizenship_id: driver.citizenship_id,
  ...
  prepareRequest() {
    ...
    request.citizenship_id = this.form.get('born_country_id')?.value; //aniqlashtirishi kerak uida fuqarolik yuq
