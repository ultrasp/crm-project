...
@Component({
  selector: 'crm-address-panel',
  templateUrl: './CrmAddressPanel.html',
  styleUrls: ['./CrmAddressPanel.css'],
})
export class CrmAddressPanel implements OnInit {
  ...
  prepareSaveRequest(){
    ...
    if(this.form.get('street')?.value)
      request.street_name = this.form.get('street')?.value;
  ...
  getAddressText():string{
    ...
    text += ', НП. '+ this.form.get('livePlace')?.value

    if(this.form.get('street')?.value)
    text += ', Ул. '+ this.form.get('street')?.value
