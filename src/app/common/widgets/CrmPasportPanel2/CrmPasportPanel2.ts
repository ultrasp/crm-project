...
@Component({
  selector: 'crm-passport-panel-2',
  templateUrl: './CrmPasportPanel2.html',
  styleUrls: ['./CrmPasportPanel2.css']
})
export class CrmPasportPanel2 implements OnInit {
  ...
  async fillData(){
    ...
    request.setTypeId(this.form.get('docType')?.value);
    ...
    if (response) {
      ...
      if (items.length > 0) this.setValue(items[0])
    }
